...
class OfferDAOImpl implements OfferDAO
{
    ...
    {
        $command = "UPDATE offers SET title='" . $offer->getTitle() . "', subtitle='" . $offer->getSubTitle() . "', companyname='" . $offer->getCompanyName() . "', description='" . $offer->getDescription() . "',free='" . $offer->getFree() . "',offerType='" . $offer->getOfferType() . "',duration='" . $offer->getDuration() . "',workModel='" . $offer->getWorkModel() . "' WHERE id='" . $offer->getId() . "'";
        return $this->database->execute($command);
    }
